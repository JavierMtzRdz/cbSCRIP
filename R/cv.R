...
run_cv_fold <- function(fold_indices, cb_data,
    ...
    for (i in seq_along(lambdagrid)) {
        ...
        non_zero[i] <- sum(!same(rowSums(abs(coef_mat[penalized_rows, , drop = FALSE])), 0))
...
cv_cbSCRIP <- function(formula, data, regularization = "elastic-net",
    ...
    non_zero_matrix <- lapply(fold_list, function(.x) {
        .x$non_zero
    })
...
print.cbSCRIP.cv <- function(x, ...) {
    ...
    cat(paste0(
        "Performed ", n_folds, "-fold cross-validation over ",
...
print.cbSCRIP.path <- function(x, ...) {
    ...
    cat(paste0(
        "Number of non-zero coefficients: from ",
...
refit_cbSCRIP <- function(object, ...) {
    ...
    refitted_coefficients <- purrr::map(refitted_models, function(model) {
        ...
        full_coef_mat <- matrix(0,
            nrow = p_orig, ncol = K_orig,
            dimnames = list(
                rownames(orig_coef_template),
...
cbSCRIP <- function(formula, data, regularization = "elastic-net",
    ...
    if (nlambda > 1) {
        ...
        # Count unique variables with at least one non-zero coef, excluding unpenalized terms
        non_zero <- purrr::map(path_fits, function(.x) {
            coef_mat <- .x$coefficients
            penalized_rows <- !rownames(coef_mat) %in% c("(Intercept)", "log(time)")
            sum(!same(rowSums(abs(coef_mat[penalized_rows, , drop = FALSE])), 0))
        })
